...
renderPlot({
  if(input$Type == "Numbers") {
    gg_bubble(d$rbys |> filter(year == 2022), input$Species, N, cl = d$cl) + theme(legend.position = "none")
  } else {
    gg_bubble(d$rbys |> filter(year == 2022), input$Species, B, lab = "Catch [kg]", cl = d$cl) + theme(legend.position = "none")
  }
})
```
